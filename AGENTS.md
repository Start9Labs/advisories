...
- **Confirm it is an advisory.** A confirmed situation people are hitting now, with a remedy, that will pass. A bug report belongs in its code repository's tracker; evergreen guidance belongs in the docs; a suspicion goes to a human, as an issue here.
...
One advisory per PR, titled `advisory <n>: <title>`. The body says where the situation was seen — the support thread, the upstream issue, the release — and, for an update, what changed and why.
